    ...
    static func loadSampleAppointments() -> [Appointment]? {
        ...
        if let seekers = Seeker.loadFromFile(),
            ...
            return [Appointment(patient: (seekers[0]), doctor: (doctors[0]), appointmentDate: dateFromString1!, status: Status.attended, issue: "Anxiety and Depression"),
                    ...
                    Appointment(patient: (seekers[2]), doctor: (doctors[1]), appointmentDate: dateFromString1!, status: Status.cancelled, issue: "Overthinking and Anxiety"),
                    
                    Appointment(patient: (seekers[0]), doctor: (doctors[3]), appointmentDate: dateFromString1!, status: Status.attended, issue: "Stress issues"),
                    
                    Appointment(patient: (seekers[3]), doctor: (doctors[3]), appointmentDate: dateFromString1!, status: Status.notAttend, issue: "Stress issues")
            ]
        }
        return nil
    }
